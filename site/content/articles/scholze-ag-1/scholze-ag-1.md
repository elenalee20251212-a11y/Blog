...
**定理 11.5 的证明**：首先处理第 1 部分。设 $M$ 是 $A$-模，$\mathcal{N}$ 是 $\mathcal{O}_X$-模层，声称以下映射是一个双射：$$\operatorname{Hom}_{\mathcal{O}_X}(\widetilde{M}, \mathcal{N}) \longrightarrow \operatorname{Hom}_A(M, \mathcal{N}(X)) \tag{11.8}$$若设 $\mathcal{N} = \widetilde{N}$（对某个 $A$-模 $N$），即可得到定理第 1 部分的结论（利用 $\widetilde{N}(X) = N$）。从范畴论的角度，这蕴含了 $A$-模范畴与 $\mathcal{O}_X$-模层范畴之间的伴随关系，其左伴随为 $M \mapsto \widetilde{M}$，右伴随为 $\mathcal{M} \mapsto \mathcal{M}(X)$。

**单射性**：设 $\phi, \phi': \widetilde{M} \to \mathcal{N}$ 是两个 $\mathcal{O}_X$-模层映射，且满足 $\psi = \phi(X) = \phi'(X) : M \to \mathcal{N}(X)$。对于每个开子集 $U = D(f) \subseteq X$（其中 $f \in A$），有以下交换图：
```tikz {embedFontCss=true}
\begin{tikzcd}
...
**命题 11.9**：
给定环态射 $g : A \to B$，一个 $A$-模 $M$ 和一个 $B$-模 $N$，有如下伴随同构：
$$
\operatorname{Hom}_A(M, N) \cong \operatorname{Hom}_B(M \otimes_A B, N)
$$
其中左侧的 $N$ 通过 $g$ 赋予了 $A$-模结构。
...
取 $X$ 的由仿射开集 $U_i = D(f_i)$ 组成的覆盖，它给出了 $D(g)$ 的一个覆盖 $D(f_i) \cap D(g) = D(f_i g)$。因为 $X$ 是拟紧的，我们只需要有限个这样的开集。

利用性质 $\mathcal{M}(D(f_i g)) = M_i[g^{-1}]$（源于 $\mathcal{M}|_{D(f_i)} = \widetilde{M_i}$），我们可以将 $M$ 表示为均等子（equalizer）：$$\begin{aligned}M &= \operatorname{eq}\left( \bigoplus_i \mathcal{M}(D(f_i)) \rightrightarrows \bigoplus_{i,j} \mathcal{M}(D(f_i f_j)) \right) \\&= \operatorname{eq}\left( \bigoplus_i M_i \rightrightarrows \bigoplus_{i,j} M_{i,j} \right)\end{aligned}$$由于局部化（localization）是精确的（exact），我们有：$$\begin{aligned}M[g^{-1}] &= \operatorname{eq}\left( \bigoplus_i \mathcal{M}(D(f_i g)) \rightrightarrows \bigoplus_{i,j} \mathcal{M}(D(f_i f_j g)) \right) \\&= \operatorname{eq}\left( \bigoplus_i M_i[g^{-1}] \rightrightarrows \bigoplus_{i,j} M_{i,j}[g^{-1}] \right) \\&= \mathcal{M}(D(g))\end{aligned}$$最后一个等号利用了 $\mathcal{M}$ 是一个层的性质。证毕。 $\square$

**推论 11.10**： [粘合模 (Gluing Modules)]
从 $A$-模范畴到下述范畴的子范畴等价：由 $A[f_i^{-1}]$-模 $M_i$ 构成的集合以及满足余圈条件（cocycle condition）的同构 $\alpha_{ij} : M_i[f_j^{-1}] \to M_j[f_i^{-1}]$ 组成的范畴，对应的映射为 $M \mapsto M_i = M[f_i^{-1}]$。
...
**定义 11.11**：概形态射 $f : Z \to X$ 称为闭浸入（closed immersion），如果它在拓扑空间上诱导的映射是一个闭浸入（即到闭子空间的同胚），且层映射 $f^\sharp : \mathcal{O}_X \to f_* \mathcal{O}_Z$ 是一个满射。

**命题 11.12**：设 $f : Z \to X$ 是概形之间的态射，则以下条件等价：
1. $f$ 是闭浸入。
2. 对所有仿射开子集 $U \subseteq X$（满足 $U = \operatorname{Spec} A$），其逆像 $f^{-1}(U) = \operatorname{Spec} B \subseteq Z$ 是仿射开集，且环态射 $A \to B$ 是满射。
...
**定义 11.14**：空间 $X$ 被称为拟紧（quasi-compact）的，如果它的每一个开覆盖都存在有限子覆盖；被称为拟分离（quasi-separated）的，如果给定两个拟紧开子集 $U$ 和 $V$，其交集 $U \cap V$ 也是拟紧开子集。概形态射被称作拟紧（或拟分离）的，如果拟紧（或拟分离）开子集的逆像也是拟紧（或拟分离）的。
...
**证明**：我们可以假定 $X$ 是仿射的，即 $X = \operatorname{Spec} A$ 是拟紧且拟分离的。由假设 $Y$ 也是拟紧且拟分离的，因此 $Y = \bigcup_i \operatorname{Spec} B_i$ 是有限个环 $B_i$ 谱的有限并。对所有的 $i$ 和 $j$，$\operatorname{Spec} B_i \cap \operatorname{Spec} B_j \subseteq Y$ 可以写为有限并：$$\bigcup_{k \in J_{ij}} \operatorname{Spec} B_{i,j,k}$$令 $M = (f_* \mathcal{N})(X) = \mathcal{N}(Y)$，我们要证明映射 $\phi : \widetilde{M} \to f_* \mathcal{N}$ 是一个同构。对所有 $g \in A$，我们要证明：$$\phi(D(g)) : M[g^{-1}] \longrightarrow (f_* \mathcal{N})(D(g))$$是同构。与定理 11.5 第 2 部分的证明类似，我们有：$$M = \mathcal{N}(Y) = \operatorname{eq}\left( \prod_i \mathcal{N}(\operatorname{Spec} B_i) \rightrightarrows \prod_{i,j, k \in J_{ij}} \mathcal{N}(\operatorname{Spec} B_{i,j,k}) \right)$$由于 $J_{ij}$ 是有限集，这些直积（products）等同于直和（direct sums）。因为局部化是精确的，我们得到：$$M[g^{-1}] = \operatorname{eq}\left( \prod_i \mathcal{N}(\operatorname{Spec} B_i)[g^{-1}] \rightrightarrows \prod_{i,j, k \in J_{ij}} \mathcal{N}(\operatorname{Spec} B_{i,j,k})[g^{-1}] \right)$$由于 $\mathcal{N}$ 是拟凝聚层以及它满足层的性质，可得：$$\begin{aligned}M[g^{-1}] &= \operatorname{eq}\left( \prod_i \mathcal{N}(\operatorname{Spec} B_i[g^{-1}]) \rightrightarrows \prod_{i,j, k \in J_{ij}} \mathcal{N}(\operatorname{Spec} B_{i,j,k}[g^{-1}]) \right) \\&= \mathcal{N}(f^{-1}(D(g))) \\&= (f_* \mathcal{N})(D(g))\end{aligned}$$因此 $\widetilde{M} \to f_* \mathcal{N}$ 是一个同构。证毕。 $\square$

**命题 11.12 的证明**：
**第 3 部分蕴含第 1 部分**：
    我们可以在局部进行验证。层的满射可在茎上验证，而空间的闭映射也可以在覆盖上验证。
    假设 $X = \operatorname{Spec} A$ 且 $Z = \operatorname{Spec} B$，令 $p : A \twoheadrightarrow B$ 为满同态。
    设 $I \subseteq A$ 是该满射 $p$ 的核，则有 $|\operatorname{Spec} B| = V(I) \subseteq |\operatorname{Spec} A|$，而 $V(I)$ 在 $\operatorname{Spec} A$ 中是闭集。
    此外，对于所有的 $g \in A$，我们有：
    $$
    A[g^{-1}] = \mathcal{O}_X(D(g)) \longrightarrow (f_* \mathcal{O}_Z)(D(g)) = \mathcal{O}_{\operatorname{Spec} B}(D(p(g))) = B[p(g)]^{-1}
    $$
    由于局部化是精确的，上述映射 $A[g^{-1}] \to B[p(g)]^{-1}$ 是满射。

**第 1 部分蕴含第 2 部分**：
    同样在局部进行验证。设 $X = \operatorname{Spec} A$，因为 $|Z| \subseteq |X|$ 是闭的，这说明 $Z$ 也是拟紧且拟分离的（因为 $X$ 是仿射的，因而是拟分离的）。
    命题 11.15 此时保证了 $f_* \mathcal{O}_Z$ 是拟凝聚层，所以有：
    $$
    f_* \mathcal{O}_Z \cong \widetilde{B}
    $$
    其中 $B = (f_* \mathcal{O}_Z)(X) = \mathcal{O}_Z(Z)$ 是一个 $A$-代数。
    我们想证明 $Z = \operatorname{Spec} B$。已知我们有一个满射 $\mathcal{O}_X \to f_* \mathcal{O}_Z$。根据练习卷 6 的第 3 题，若 $X = \operatorname{Spec} A$，则 $\mathcal{O}_X$-模序列是精确的，当且仅当其全局截面序列作为 $A$-模是精确的。这说明我们有满射 $A \to B$。
    因此，$A$-代数映射 $B \to \mathcal{O}_Z(Z)$ 诱导了一个在 $\operatorname{Spec} A$ 之上的概形态射 $\phi : Z \to \operatorname{Spec} B$。接下来只需证明 $\phi$ 是一个同构。
    若存在某个 $x \in \operatorname{Spec} B \setminus Z$，那么恒有 $0 = (f_* \mathcal{O}_Z)_x = \widetilde{B}_x \neq 0$（因为 $x \in \operatorname{Spec} B$），产生矛盾。因此在拓扑空间上有 $|Z| = |\operatorname{Spec} B|$。
    对于层，通过评估可得：
    $$
    \mathcal{O}_Z(D(g) \cap Z) = (f_* \mathcal{O}_Z)(D(g)) = \widetilde{B}(D(g)) = B[g^{-1}] = \mathcal{O}_{\operatorname{Spec} B}(D(g) \cap Z)
    $$
    由此得到 $\mathcal{O}_Z = \mathcal{O}_{\operatorname{Spec} B}$。证毕。 $\square$

本部分PDF内容（第47至54页）主要涵盖了第12节“向量丛与Picard群”以及第13节“有限性条件与维度”。以下是详细、完整的简述：
