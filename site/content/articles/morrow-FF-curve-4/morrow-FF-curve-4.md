...
构造 Huber 环最简单的方法是：从一个环 $R_0$ 和一个非零因子 $\pi \in R_0$ 开始，并令：$$R := R_0\left[\frac{1}{\pi}\right]$$拓扑基由如下形式的集合给定：$$f + \pi^m R_0 \quad (\text{其中 } f \in R, m \ge 0)$$这样的 Huber 环 $R$ 被称为 **Tate 环**。
...
> 注: 拓扑环的子集 $B$ 有界定义为: 对任何 $0$ 的邻域 $U$ , 都存在 $0$ 的邻域 $V$ , $VB\subset U$ .

##### 关于 $R^+$ 的选择
有时取 $R^+ = R_0$，但其他时候这甚至是不可行的（因为 $R_0$ 可能不是整闭的）。另一方面，总有最大的一个 $R^+$ ，即 $R$ 中所有幂有界元素组成的子环。
...
因为 diamond 范畴具有乘积，我们可以形成新的 diamond $\mathrm{Spd}(\mathbb{Q}_p) \times \mathrm{Spd}(\mathbb{Q}_p)$，它的 $\underline{S}$-点是 $\underline{S}$ 的 untilt 对（等价类对）。

##### 更一般的点函子形式
更一般地，给定任何 Tate-Huber 对 $\underline{R} = (R, R^+)$，我们可以考虑 $\underline{S}$ 在 $\underline{R}$ 之上的 untilt：由定义，这样的 untilt 是一个三元组 $(\underline{T}, \iota, f)$，包含一个 perfectoid Tate-Huber 对 $T$，一个同构 $\iota : \underline{S} \overset{\sim}{\to} \underline{T}^\flat$，以及一个同态 $f : \underline{R} \to \underline{T}$。与 $\mathrm{Spd}(\underline{R})$ 关联的点函子精确为：$$\underline{S} \mapsto \{ \underline{S} \text{ 在 } \underline{R} \text{ 之上的 untilt 的等价类} \}$$利用这一点，我们可以验证：给定任何 perfectoid Tate-Huber 对 $T$，存在自然的 diamond 同等：$$\mathrm{Spd}(\underline{T}) = \mathrm{Spd}(\underline{T}^\flat)$$实际上，这可以由以下倾斜等价性推出：

> 这里具体来说是, 按上面的描述$$\operatorname{Spd}(\underline T)(\underline S)=\left\{(\underline U,\iota,f)|\iota:\underline S\overset\sim\to\underline U^\flat,f:\underline T\to\underline U\right\}/\sim$$有以下$$\begin{aligned}\operatorname{Spd}(\underline T)(\underline S)&\cong\{\text{untilts of }\underline S\text{ over }\underline T\}/\sim\\&\cong\operatorname{Hom}_{\mathrm{Aff\text{-}Perf}}(\underline T^\flat,\underline S)\\&\cong\operatorname{Hom}_{\mathrm{Diam}}(\operatorname{Spd}(\underline S),\operatorname{Spd}(\underline T^\flat))\\&=\operatorname{Spd}(\underline T^\flat)(\underline S).\end{aligned}$$
...
它分类了 $\mathbb{C}((t))^n$ 内部的 $\mathbb{C}[[t]]$-格（lattices），具有 ind 复解析空间的结构；或者从代数观点看，以下函子由一个 ind 射影 scheme 表示（这在几何 Langlands 纲领与几何 Satake 对应中是基础的）：
...
更一般地，给定任何完美 Tate-Huber 对 $\underline{S}$ 和选定的 $C$ 之上的 untilt（即态射 $\mathrm{Spd}(\underline{S}) \to \mathrm{Spd}(C)$），可以定义这些 de Rham 周期环的对应物，从而在 $\mathrm{Spd}(C)$ 上的完美 Tate-Huber 对范畴上定义经典的仿射格拉斯曼流形对应物。这个所谓的 **$B_{\mathrm{dR}}^+$-affine Grassmannian** 是一个 ind-diamond [32, §19]，有望出现在算术 Satake 对应中。
...
\end{tikzcd}
```
其中第一支箭头由 $f$ 诱导，同构由 $\iota$ 诱导，而 $\theta_T$ 映射是命题 5.1 中 Fontaine 映射的推广。（实际上，这整个构造与命题 5.1 的论证基本相同，只是现在用 diamond 语言表述）。反之，每一个这样的同态 $A_{\mathrm{inf}} \to T^+$ 都可以通过这种方式构造（由 Witt 向量的普遍性质），这意味着 diamond 的映射是一个同构。

##### 相对 Fargues-Fontaine 曲线
Fargues 和 Fontaine 的工作更一般地适用于条件“$F$ 是完美的”而非“$F$ 是代数闭的”。利用 perfectoid 空间和 diamond，对于特征 $p$ 的任何 perfectoid 空间 $Z$，存在关联的 Fargues-Fontaine “关于 $\mathbb{Z}$ 的相对曲线”：$$X^{\mathrm{FF}}_Z = \mathcal{Y}_Z/\varphi^{\mathbb{Z}}$$对象 $\mathcal{Y}_Z$ 和 $X^{\mathrm{FF}}_Z$ 依然是 adic 空间，其关联的 diamond 满足前述命题的类似公式。
