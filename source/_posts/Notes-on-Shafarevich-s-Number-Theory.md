...
$\mathrm{Ex.\ 2.2.}$ 令 $k=\operatorname{Frac}(A)$ ，由于 $k[t]$ 在 $k(t)$ 中整闭（因为是UFD），故只需证 $A[t]$ 在 $k[t]$ 中整闭。假设 $f\in k[t]$ 满足关系$$f^n+a_{n-1}f^{n-1}+\cdots+a_0=0,\quad a_0,\cdots,a_{n-1}\in A[t]$$则考虑 $f$ 最高次项系数 $f_n$ ，它满足某个 $A$ 系数多项式从而属于 $A$ 。现在对 $n$ 归纳证 $f\in A[t]$ ，事实上考虑 $f-f_nt^n$ 降次即可。$\square$
